...
- ✅ 做：日线定方向 + 5 分钟价量通道定触发带 + 通道宽度看分歧/确定性 + 动能看推进/耗散 + 标准化条件单方案
...
- [ ] 1. 解析标的代码
- [ ] 2. 拉取报价（可走缓存）
- [ ] 3. 出主图：日线 / 5m K线 / 价量通道 / 通道宽度 / 动能（分时可选）
- [ ] 4. 生成标准化条件单方案 emquote plan（含宽度确定性 + 动能评估）
...
1. 报价 JSON  
2. 主图 `daily` / `kline` / `pv` / `width` / `ke`（可选 `intraday`）  
3. `levels` + **`plan`（标准化条件单方案，含宽度确定性 + 动能）**  
...
emplot-ke 603606.SH -o /tmp/ke.png --json
...
- 价量动能：`{ke.png}` —— 看推进/耗散（回踩窗口 vs 突破叙事）
